...
export class AddProcessComponent implements OnInit {
  ...
  @ViewChild ('textAreaScript') _textAreaScript!:ElementRef
  uploadingFilesInUF:Array<any> = []
  localfiles:Array<any> =[]
  uploadingFilesForm!:FormGroup
  delimeterList:Array<any> = [',',':',';','|','\t']
  ...
  desktopServicesArray: Array<any> = [{ value: 'Power BI Desktop', isChecked: false }, { value: 'Power BI Services', isChecked: false }]
  ...
  ngOnInit(): void {
    ...
    this.uploadingFilesForm = this.formBuilder.group({
  ...
  deleteAnalyticsRow(index: number) {
    // this.seniroList.removeAt(index);
    this.seniroList.splice(index, 1);
    // (this.Genericform.get("analytics") as FormArray).removeAt(index);
    // this.genericFormLength = (this.Genericform.get('analytics') as FormArray).value.length;
  }
        ...
        (res: any) => {
          ...
          if (ResponseData.length == 0) this.showUploadFilesTab = false;
            ...
            ResponseData.forEach((fileObj: any) => {
              ...
              else if(fileObj.fileExt == 'csv' || fileObj.fileExt == 'txt' ||fileObj.fileExt == 'json'){
                fileObj['File Name'] = fileObj.fileNameWithExt
              ...
              fileObj['Uploaded On'] = fileObj.uploadedOn,
  ...
  deleteUploadingFiles(i:number){
    this.uploadingFilesInUF.splice(i,1)
    this.localfiles.splice(i,1) //x working
    if(this.uploadingFilesInUF.length == 0) this.progressBarPopup = false
  }

  addFilesUF(): FormGroup{
    return this.formBuilder.group({
      file:[]
    });
  }
  addDelimeterForFilesUF():FormGroup{
    return this.formBuilder.group({
      delimeter:[]
    });
  }
  ...
  saveiputFiles(ev: any, type: any) {  //R
    ...
    if (type == 'click') this.localfiles = ev.target.files
    else if (type == 'dragNDrop') this.localfiles = ev.dataTransfer.files
    let acceptedExtentions = ['xlsx', 'xls', 'txt', 'csv', 'json', 'jpeg', 'png', 'jpg', 'pdf']

    this.uploadingFilesInUF = []
    this.uploadingFilesForm.controls.filesList = this.fb.array([]);

    this.progressBarPopup = true
    for (let index = 0; index < this.localfiles.length; index++) {
      const file = this.localfiles[index];
      if (acceptedExtentions.includes(file.name.split('.')[1])) {
        // if(file.type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        // if(condition == 'beforeUploading') this.fileListt.push({ fileNameWithExt: file.name.split('.')[0], isProgressBar: true, fileExt: file.name.split('.')[1] })
        
        this.uploadingFilesInUF.push({ file: this.localfiles[index].name})
        this.uploadFilesArr().controls.push(this.addFilesUF())
        this.uploadFilesArr().controls[index].patchValue({file: this.localfiles[index].name})
        if(file.name.split('.')[1] === 'txt'){
        (<FormGroup>this.uploadFilesArr().controls[index]).addControl('delimeter', this.fb.control([]))

          // this.uploadFilesArr().controls.push(this.addDelimeterForFilesUF())
        }
      }
    }
  ...
  hasDelimeter(i:number){
    return this.uploadFilesArr().controls[i].get('delimeter') ? true : false
  }

  uploadFilesArr(): FormArray{
    return this.uploadingFilesForm.get('filesList') as FormArray;
  }
    ...
    for (let i = 0; i < this.localfiles.length; i++) {
      const file = this.localfiles[i];
        if (acceptedExtentions.includes(file.name.split('.')[1])) {
          const formData = new FormData();
          formData.append('processId', processId);
          formData.append('file', this.localfiles[i])
            ...
            this.solService.postInputFiles(formData).pipe(
              tap((e: any) => {
                ...
                if (e.type === HttpEventType.UploadProgress) {
                  this.progressBar[i] = { percentage: Math.round((95 / e.total) * e.loaded), file: this.localfiles[i].name, isError: false, icon: false }
    ...
    this.solService.addAppendFiles(appendObj).subscribe(
      (res: any) => {
        if (res.type == HttpEventType.UploadProgress) {
          this.progressBar = [{ percentage: Math.round((100 / res.total) * res.loaded) }];
  ...
  add_SaveCreateExpression(obj: any) {
    ...
    selectedColumn = this.allTableListInDM.filter((element: any) => element.columnList.some(
    ...
    if (uploadType == 'Python Script') {
      ...
      if (
        ...
        fileReader.onload = (e: any) => {
          this.uploadSQLBtn = false;
          try{
            this.sqlScriptForm.patchValue({ sqlScript: format(JSON.parse(JSON.stringify((<string>fileReader.result).split('[').join('').split(']').join('')))) });
          }
          catch(error){
            console.log(error);
            return
...
scriptErrMsg:string = ''
  oneNext(item: any, type: any) {
    ...
    this.scriptErrMsg = ''
    try {format(this.sqlScriptForm.controls['sqlScript'].value)}
    catch(err:any){
      let er = err.message
      console.log(er);

      let lineNumber = Number(er.substring(er.indexOf('line') +5, er.lastIndexOf('column')))
      let columnNumber = ( er.substring(er.indexOf('column') + 7).includes('\n')) ? Number(er.substring(er.indexOf('column') + 7).split('\n')[0]) : Number(er.substring(er.indexOf('column') + 7))
      let errLine = this.sqlScriptForm.controls['sqlScript'].value.split('\n', lineNumber - 1).join('\n').length
      let particularErrChar = columnNumber + errLine
      const textAre = <HTMLTextAreaElement>this._textAreaScript.nativeElement
      textAre.focus()
      textAre.setSelectionRange(particularErrChar, particularErrChar+1)
      this.scriptErrMsg = er.substring(0,100)
      return
    }
...
targetFileNextCndn(): any {
  let hasColumns = this.targetFileList.filter((element: any) =>
    element.sheet.some(
      (subElement: any) => subElement.isChecked == true
    )
  )
