...
export class ViewSubscriptionComponent implements OnInit {
      ...
      reqCall.subscribe(
        (res: any) => {
          ...
          this.procArr().controls.forEach((element: any, i: any) => {
            this.procArr().controls[i].patchValue({
  ...
  addnew(event: any, itemIndex: number, data: any, processType: any, masterSelect?: any) {
    ...
    processType?.trim()?.toLowerCase() == 'generic'
      ...
      if (work.selectUser) {
        ...
        if (orgSubsId) {
          subsciptionType.push({
            ...
            userId: users.length > 0 ? users : null,
    ...
    this.procArr().value?.forEach?.((process: any, i: any) => {
      ...
      if (this.subscriptions[i].orgSolnId) {
        subsciption.push({
          ...
          orgSolnId: this.subscriptions[i].orgSolnId,
          usersolnMapId:
            process.selectUser == this.subscriptions[i].userID
              ? this.subscriptions[i].usersolnMapId
              : null,
        ...
        case false:
          {
            ...
            setTimeout(() => {
              if (!gp.checked) {
                this.addnew('', index, gp, 'generic', { checked: this.masterSelectCSProc });
              }
            }, 10);
          }
          break;
